...
begin
  ...
  -- If the referenced project/org no longer exists (e.g. during cascading DELETE
  -- of a project), null out the reference so the audit insert doesn't fail the
  -- foreign-key check.
  if v_proj is not null and not exists (select 1 from public.projects where id = v_proj) then
    v_proj := null;
  end if;
