...
import { CheckCircle2, XCircle, FlaskConical, Scissors, Cable as CableIcon } from "lucide-react";
      ...
      <div className="animate-fade-in space-y-6">
        <header>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Zakončování, testování a uzavírání natažených kabelů.
          </p>
        </header>
            ...
            return (
              <div key={col.key} className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  {items.map((t) => (
                    <Card key={t.id} className="animate-fade-in border-border/60 transition-all hover:border-primary/40 hover:shadow-sm">
                      <CardContent className="space-y-2 p-3">
                        <div className="font-mono text-xs font-semibold">{t.cableCode}</div>
                        {t.cancelledReason && (
                          <div className="rounded bg-muted p-2 text-[10px] text-muted-foreground">
                            {t.cancelledReason}
                          </div>
                        )}
                        {col.next.length > 0 && (
        ...
        <Dialog open={cancelFor !== null} onOpenChange={(o) => !o && setCancelFor(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Zrušit kabel</DialogTitle>
            </DialogHeader>
            <div>
              <label className="mb-1 block font-mono text-xs uppercase text-muted-foreground">Důvod</label>
